...
<div class="dropdown-menu dropdown-menu-end">
	<form action="{{ route('admin.activities.logs') }}" id="filters-form">
		...
		<div class="dropdown-item">
			<div class="form-group mb-0">
				...
				<select class="select2 form-select" name="users[]" data-placeholder="Nothing selected" multiple>
			      	@foreach($users as $c)
			      		<option value="{{ $c->id }}" {{ isset($_GET['users']) && in_array($c->id, $_GET['users'])  ? 'selected' : '' }}>{{ $c->first_name . ' ' . $c->last_name }}</option>
			  		@endforeach
			    </select>
			</div>
		</div>
		...
		<div class="dropdown-item">
			<div class="form-group mt-2 mb-0">
				...
				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="role" id="allrole" value="" {{ (!isset($_GET['role']) || $_GET['role'] === '' || $_GET['role'] === null ? 'checked' : '') }}/>
					<label class="form-check-label" for="allrole">All</label>
				</div>
				<div class="form-check form-check-inline">
					<input type="radio" class="form-check-input" name="role" id="onlyrole" value="client" {{ (isset($_GET['role']) && $_GET['role'] == 'client' ? 'checked' : '') }}/>
					<label class="form-check-label" for="onlyrole">Only Client</label>
				</div>
				<div class="form-check form-check-inline">
					<input type="radio" class="form-check-input" name="role" id="superrole" value="admin" {{ (isset($_GET['role']) && $_GET['role'] == 'admin' ? 'checked' : '') }}/>
					<label class="form-check-label" for="superrole">Only Admins</label>
				</div>
			</div>
		</div>
		<div class="dropdown-divider"></div>
		<div class="dropdown-bottom clearfix">
			<a href="{{ route('admin.activities.logs') }}" class="btn btn-sm btn-danger px-3 float-start">Reset All</a>
